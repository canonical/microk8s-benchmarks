...
def calculate_shift(current, target):
    cps_diff = target["cps"] - current["cps"]
    wks_diff = target["wks"] - current["wks"]
    return cps_diff, wks_diff


@pytest.mark.parametrize(
    "current,target,expected",
    [
        (current, dict(total=10, cps=5, wks=5), (0, 0)),
        (current, dict(total=10, cps=3, wks=7), (-2, 2)),
        (current, dict(total=10, cps=7, wks=3), (2, -2)),
        (current, dict(total=15, cps=5, wks=10), (0, 5)),
        (current, dict(total=15, cps=10, wks=5), (5, 0)),
        (current, dict(total=15, cps=7, wks=8), (2, 3)),
        (current, dict(total=15, cps=1, wks=14), (-4, 9)),
        (current, dict(total=7, cps=2, wks=5), (-3, 0)),
        (current, dict(total=7, cps=5, wks=2), (0, -3)),
        (current, dict(total=7, cps=4, wks=3), (-1, -2)),
        (current, dict(total=1, cps=1, wks=0), (-4, -5)),
    ],
)
def test_shift(current, target, expected):
    try:
        assert calculate_shift(current, target) == expected
    except AssertionError:
        import pdb

        pdb.set_trace()
        calculate_shift(current, target)
